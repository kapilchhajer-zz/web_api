...
def polish():
    ...
    with lcd(ROOT_DIR):
        with settings(hide('running'), warn_only=True):
            ...
            result = local('pyflakes core/app ./fabfile.py')
            found_errors = found_errors or result.return_code != 0
            ...
            result = local(
                '! find core/app -type f -name "*.py" -print0 | '
            ...
            result = local(
                '! find core/app -name "settings.py" -o '
            ...
            result = local('! alembic branches | grep branchpoint')
            found_errors = found_errors or result.return_code != 0
